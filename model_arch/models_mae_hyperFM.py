...
from model_arch.gating_function import TopKGroupRouter

class HyperFM(nn.Module):
    """ Masked Autoencoder HyperFM model
    Args:
        ...
        spatial_mask (bool): Whether to mask all channels of same spatial location. Default: False
    ...
    def __init__(self, img_size=96, patch_size=8, in_chans=291, spatial_mask=False,
        ...
        self.decoder_blocks = nn.ModuleList([
            HypoformerBlock(decoder_embed_dim, decoder_num_heads, mlp_ratio, beta, tt_rank, mf_rank)
    ...
    def forward_encoder(self, x, mask_ratio):
        ...
        for i, group in enumerate(self.channel_groups):
            x_c = x[:, group, :, :]
            x_c_gba.append(self.group_band_attn[i](x_c))  # (G, B, C_i, H, W)

        x = torch.cat(x_c_gba, dim=1)  # (B, C, H, W)
        ...
        for i, group in enumerate(self.channel_groups):
            x_c = x[:, group, :, :]
            x_c_embed.append(self.patch_embed[i](x_c))  

        x = torch.stack(x_c_embed, dim=1) 
        _, G, L, D = x.shape

        # add channel embed
        channel_embed = self.channel_embed.unsqueeze(2)  
        pos_embed = self.pos_embed[:, 1:, :].unsqueeze(1)  
        ...
        if self.spatial_mask:
            ...
            x = x.permute(0, 2, 1, 3).reshape(b, L, -1)  # (B, L, G*D)
            x, mask, ids_restore = self.random_masking(x, mask_ratio)  # (B, 0.25*L, G*D)
            x = x.view(b, x.shape[1], G, D).permute(0, 2, 1, 3).reshape(b, -1, D)  # (B, 0.25*G*L, D)
            mask = mask.repeat(1, G)  # (B, G*L)
            mask = mask.view(b, G, L)
        else:
            # Independently mask each channel (i.e. spatial location has subset of channels visible)
            x, mask, ids_restore = self.random_masking(x.view(b, -1, D), mask_ratio)  # (B, 0.25*G*L, D)
            mask = mask.view(b, G, L)
        # append cls token
        cls_tokens = self.cls_token.expand(x.shape[0], -1, -1)
        x = torch.cat((cls_tokens, x), dim=1)  # (B, G*L + 1, D)
    ...
    def forward(self, imgs, mask_ratio=0.75):
        ...
        return loss, None, pred, mask

################################################################################

def mae_hyp_small_patch8_dec512d8b(**kwargs):
    model = HyperFM(
        channel_embed=256, embed_dim=768, depth=4, num_heads=12,
        decoder_channel_embed=128, decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model

def mae_hyp_base_patch8_dec512d8b(**kwargs):
    model = HyperFM(
...
def mae_hyp_large_patch8_dec512d8b(**kwargs):
    model = HyperFM(
        channel_embed=256, embed_dim=1024, depth=24, num_heads=16,
        decoder_channel_embed=128, decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
...
hyperFM_enc4 = mae_hyp_small_patch8_dec512d8b  # encoder: 768 dim, 4 blocks, decoder: 512 dim, 8 blocks
hyperFM_enc12 = mae_hyp_base_patch8_dec512d8b  # encoder: 768 dim, 12 blocks, decoder: 512 dim, 8 blocks
hyperFM_enc24 = mae_hyp_large_patch8_dec512d8b  # encoder: 1024 dim, 24 blocks, decoder: 512 dim, 8 blocks